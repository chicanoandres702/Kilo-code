...
function base(ver: string, kind: Kind, tags: Tag[]) {
  ...
  const prior = tags.filter((item) => !semver.prerelease(item.version) && semver.lt(item.version, ver))
    .sort((a, b) => semver.rcompare(a.version, b.version))

  if (kind === "stable") return prior[0]?.tag ?? null

  const prerelease = tags.filter((item) => {
      const parsed = semver.parse(item.version)
      return parsed?.major === want.major && parsed?.minor === want.minor && parsed?.patch === want.patch &&
             Boolean(semver.prerelease(item.version)) && semver.lt(item.version, ver)
    })
    .sort((a, b) => semver.rcompare(a.version, b.version))
