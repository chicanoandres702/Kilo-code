...
gate_4() {
    [[ -f "tsconfig.json" ]] && npx tsc --noEmit --strict 2>&1 | head -5 || echo "✓ Gate 4 (no TS)"
}

# Gate 5: Wiring
gate_5() {
    [[ -f "package.json" ]] && npm run lint 2>/dev/null || echo "✓ Gate 5"
}

# Gate 6: Test
gate_6() {
    [[ -f "package.json" ]] && npm test -- --coverage 2>/dev/null || echo "✓ Gate 6"
}

# Gate 7: Complexity
gate_7() {
    local violations=0
    for f in $(git diff --name-only 2>/dev/null || find src -name "*.kt" -o -name "*.ts" 2>/dev/null); do
        [[ -f "$f" ]] || continue
